...
if __name__ == '__main__':
    #unittest.main()
    morphologies = [['名詞', '名詞', '名詞', '名詞', '名詞', '接頭辞', '名詞', '名詞', '接頭辞', '名詞', '名詞', '名詞', '接頭辞', '名詞', '接尾辞', '接頭辞', '名詞', '接尾辞']]
    words = [['仮想', '関数', '擬似', '乱数', '系列', '非', '同期', '通信', '全', '二', '重', '接続', '再', '初期', '化', '未', '定義', '型']]

    tb = build_frequency_table(morphologies, words)
    print(tb)

    containing_tb = build_containing_term_table(tb)
    print(containing_tb)

    cvalue_table = build_cvalue_table(tb, containing_tb)
    print(cvalue_table)
# %%
